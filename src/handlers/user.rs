...
// タプル構造体定義を利用した別名定義。単一戻り値、複数戻り値を共通化できる
#[derive(Debug, Deserialize, Serialize, PartialEq)]
...
pub async fn get_user(
    ...
    // FIXED: let user = block(move || find(&pool, *user_id)).await?;
...
pub async fn get_users(pool: Data<PoolType>) -> Result<Json<UsersResponse>, ApiError> {
    // FIXED: let users = block(move || get_all(&pool)).await?;
...
pub async fn create_user(
    ...
    crate::validate::validate(&params)?;
    ...
    .into();
    // FIXED: let user = block(move || create(&pool, &new_user)).await?;
...
pub async fn delete_user(
    ...
    // FIXED: block(move || delete(&pool, *user_id)).await?;
...
pub mod tests {
    use super::*;
    use crate::{
        models::user::tests::create_user as model_create_user,
        tests::helpers::tests::{
            get_data_pool,
            get_pool,
        },
    ...
    async fn it_doesnt_find_a_user() {
        ...
        let expected_error = ApiError::NotFound(format!("User {} not found", uuid));
