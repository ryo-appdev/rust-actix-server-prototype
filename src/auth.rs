...
impl PrivateClaim {
    pub fn new(user_id: Uuid, email: String) -> Self {
        Self {
            user_id,
            email,
            exp: (Utc::now() + chrono::Duration::hours(CONFIG.jwt_expiration)).timestamp(),
        }
    }
}
...
pub fn decode_jwt(token: &str) -> Result<PrivateClaim, ApiError> {
    ...
    jsonwebtoken::decode::<PrivateClaim>(token, &decoding_key, &Validation::default())
...
pub fn hash(password: &str) -> String {
    argon2rs::argon2i_simple(password, &CONFIG.auth_salt)
        .iter()
        .map(|b| format!("{:02x}", b))
        .collect()
}
